...
from descriptor import PortDescr
from log.server_log_config import LOG_SERVER
from log_decor import log
from meta import ServerVerifier
from utilites.utils import get_mess, send_mess
from utilites.variables import DEFAULT_PORT, DESTINATION, ACTION, USER, ACCOUNT_NAME, SENDER, MESSAGE, MESSAGE_TEXT, \
    RESPONSE_200, RESPONSE_400, ERROR, PRESENCE, TIME, EXIT, GET_CONTACTS, RESPONSE_202, ALERT, ADD_CONTACT, USER_ID, \
    DEL_CONTACT
...
class Server(metaclass=ServerVerifier):
        ...
        while True:
            ...
            if recv_data_lst:
                for client_with_message in recv_data_lst:
                    try:
                        self.process_client_message(get_mess(client_with_message), client_with_message)
                    except:
    ...
    def process_message(self, message, listen_socks):
        if message[DESTINATION] in self.names and self.names[message[DESTINATION]] in listen_socks:
            send_mess(self.names[message[DESTINATION]], message)
