...
class ServerDB:
    ...
    def get_contacts(self, client_login):
        ...
        query = self.sess.query(ContactList, Client).filter_by(id_owner=user.id).join(Client, ContactList.id_client == Client.id)
    ...
    def add_contact(self, client, contact):
        ...
        if not user or not contact or self.sess.query(ContactList).filter_by(id_owner=user.id, id_client=contact.id).count():
            return
    ...
    def del_contact(self, client, contact):
        ...
        if not user or not contact:
            return
        self.sess.query(ContactList).filter(ContactList.id_owner == user.id, ContactList.id_client == contact.id).delete()
        self.sess.commit()
...
#     storage = ServerDB()
#     storage.add_user('Guest 1', 'Info 1')
#     storage.add_user('Guest 2', 'Info 2')
#     storage.add_user('Guest 3', 'Info 3')
#     storage.add_user('Guest 3', 'Info 3')
#     print('RESULT 1', list(storage.users_list()))
#     storage.add_contact('Guest 1', 'Guest 2')
#     storage.add_contact('Guest 1', 'Guest 3')
#     print('RESULT 2', storage.get_contacts('Guest 1'))
#     storage.del_contact('Guest 1', 'Guest 2')
#     print('RESULT 3', storage.get_contacts('Guest 1'))
